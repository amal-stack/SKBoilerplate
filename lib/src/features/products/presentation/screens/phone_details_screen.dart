  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider(create: (context) => PricingCubit(context.read())),
      ],
      child: Scaffold(
        ...
        body:
            BlocListener<ProductVariantsCubit, ViewState<List<ProductVariant>>>(
              listener: (context, state) {
                if (state case ViewSuccess<List<ProductVariant>>(
                  data: final variants,
                )) {
                  if (variants.isNotEmpty) {
                    context.read<PricingCubit>().fetchPricing(
              ...
              child:
                  ViewBlocBuilder<ProductVariantsCubit, List<ProductVariant>>(
                    builder: (context, variants) => variants.isEmpty
                        ? Center(
                            child: RegularText(
...
class _PhoneDetailsBodyState extends State<_PhoneDetailsBody> {
  ...
  int? _page;
  int? _expandedIndex;
  late final PricingCubit cubit = context.read<PricingCubit>();
  ...
  void _onPageChanged() {
    ...
    if (newIndex != null && newIndex != _selectedIndex) {
      ...
      cubit.fetchPricing(variantId: widget.variants[newIndex].id);
    }
  }
  ...
  ) => BlocListener<PricingCubit, ViewState<DevicePricing>>(
    listener: (context, state) {
      if (state case ViewSuccess<DevicePricing>(:final data)) {
        QuestionnairePriceStore.instance.saveMaximumDevicePrice(
          data.id!,
          data.price!,
        );
      }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 20.h),
              BlocBuilder<PricingCubit, ViewState<DevicePricing>>(
                builder: (context, state) => custom.RichText(
                  texts: [
                    TextModel(
                      AppTexts.getUpTo,
                      size: 12.sp,
                    ...
                    TextModel(
                      switch (state) {
                        ViewSuccess(:final data) => " ₹ ${data.price}",
                        ViewLoading() => '...',
                        _ => '',
                      },
                      size: 24.sp,
